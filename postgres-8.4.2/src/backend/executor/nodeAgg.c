...
approx_agg_init(AggState *aggstate)
{
	...
	aggstate->k = agg->approx_nkeep;
	aggstate->topKArray = palloc0(sizeof(ApproxTopEntry)*aggstate->k);
        aggstate->arraySize = 0;
        aggstate->lowestEntryIndex = -1;
        aggstate->lowestEntryCount = 2147483647;
        aggstate->sketch = init_sketch(agg->cm_width, agg->cm_depth);
	aggstate-> currentIter = 0;
...
approx_agg_per_input(AggState *aggstate, TupleTableSlot* outerSlot, Agg* agg)
{
	...
	est = estimate(aggstate->sketch, hash_values);
        set_approx_top_entry_from_slot(outerSlot,entry);
	entry->count = est;
	size = aggstate->arraySize;
	inArray = false;

	//check if tuple is in the array already
	for(i = 0; i < size; i++){
			temp = aggstate->topKArray[i];
			inArray = compare_tuple_with_approx_top_tuple(outerSlot,temp,aggstate,agg);
			if(inArray == true){
				temp->count = est;
				break;
			}
	...
	if(inArray == false){
		if(size < aggstate->k){
	 		aggstate->topKArray[size] = entry;
			if(est < aggstate->lowestEntryCount){
				aggstate->lowestEntryIndex = size;
				aggstate->lowestEntryCount = est;
			}
			aggstate->arraySize+=1;	
		}
		else{
			if(est > aggstate->lowestEntryCount){
				temp = aggstate->topKArray[aggstate->lowestEntryIndex];
				pfree(temp);
				aggstate->topKArray[aggstate->lowestEntryIndex] = entry;
				//find the new lowest count/index
				checkIndex = -1;
				checkCount = 2147483647;
				for(i = 0; i < size; i++){
					temp = aggstate->topKArray[i];
					if(temp->count < checkCount){
						checkCount = temp->count;
						checkIndex = i;
					}
				}
				aggstate->lowestEntryCount = checkCount;
				aggstate->lowestEntryIndex = checkIndex;
			}
		}
	}
...
agg_retrieve_cmsketch(AggState *aggstate)
{
	...
	while (true)
	{
		...
		aggvalues[0] = Int64GetDatum(tuple->count /* CS186-TODO: Set the count for this group here */);
...
approx_agg_advance_iter(AggState *aggstate, Agg* agg)
{
	 ...
	 */
    	int iter = aggstate->currentIter;
    	ApproxTopEntry* entry;
    	if(iter >= aggstate->arraySize){
		return NULL;
    	}
    	else{
		entry = aggstate->topKArray[iter];
		aggstate->currentIter+=1;
		return entry;
    	}
}
